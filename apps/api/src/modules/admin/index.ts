...
export const adminRoutes = new Hono<{ Bindings: Env; Variables: { jwt: JwtClaims; trace_id: string } }>()
  ...
  // Atalho de TESTE: avanca N meses de uma vez pra uma prefeitura. Determina
  // a proxima competencia sem folha (ou a atual+1 depois da ultima folha da
  // prefeitura) e faz N ciclos criar-fechada->consolidar. Cada ciclo dispara
  // o cascade parcelasPagas +1 nos contratos averbados. Nao substitui o fluxo
  // real (prefeitura abre em /prefeitura/folhas), so serve pra demo/QA.
  .post("/v1/admin/folhas/simular-meses", async (c) => {
    const j = c.get("jwt"); requireAdmin(j); requirePermissao(j, "folhas");
    const body = z.object({
      prefeituraId: z.number().int(),
      meses: z.number().int().min(1).max(60), // teto 5 anos pra evitar abuso
    }).parse(await c.req.json());
    await ensureFolhasLoaded(c.env);
    await refreshContratos(c.env);
    await Promise.all([ensurePrefeiturasLoaded(c.env), ensureBancosLoaded(c.env), refreshConvenios(c.env)]);
    ...
    if (!pref) throw Errors.notFound("prefeitura");
    // Determina a proxima competencia. Se ja ha folhas dessa prefeitura, comeca
    // pelo mes seguinte a MAIS RECENTE. Senao, comeca pelo mes atual.
    const folhasDaPref = folhas.filter((f) => f.prefeituraId === pref.id);
    const maiorComp = folhasDaPref
      .map((f) => f.competencia)
      .sort()
      .pop();
    const nextComp = (comp: string | undefined): string => {
      if (!comp || !/^\d{6}$/.test(comp)) {
        const d = new Date();
        return `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}`;
      }
      let y = Number(comp.slice(0, 4));
      let m = Number(comp.slice(4, 6));
      m += 1;
      if (m > 12) { m = 1; y += 1; }
    ...
    const ator = `averbadora:${c.get("jwt").sub}`;
    const bancoNomeById = (id: number) => bancos.find((b) => b.id === id)?.nome ?? `Banco ${id}`;
    let comp = nextComp(maiorComp);
    const criadas: { id: string; competencia: string; incrementados: number; quitados: number }[] = [];
    let totalIncrementados = 0;
    let totalQuitados = 0;
    for (let i = 0; i < body.meses; i++) {
      const nova: FolhaAdmin = {
        id: `F-${Date.now()}-${i}`,
        prefeituraId: pref.id,
        prefeitura: pref.nome,
        competencia: comp,
        dataCorte: new Date().toISOString().slice(0, 10),
        dataRepasse: null,
        status: "fechada",
      };
      folhas.push(nova);
      await persistFolha(c.env, nova);
      ensureAdfsGlobal(comp, bancoNomeById, new Date().toISOString(), [pref.id]);
      nova.status = "consolidada";
      await persistFolha(c.env, nova);
      const adfsDaFolha = listAdfsGlobal(comp).filter((a) => a.prefeituraId === pref.id && a.status === "aplicada");
      let inc = 0;
      let quit = 0;
      for (const a of adfsDaFolha) {
        const ct = getContrato(a.adf);
        ...
        inc++;
        if (ct.parcelasPagas >= ct.totalParcelas) {
          aplicarAcao(a.adf, "quitar", ator, `Quitado ao simular avanco de meses (folha ${comp}).`);
          quit++;
        }
        await persistContrato(c.env, a.adf);
      }
      totalIncrementados += inc;
      totalQuitados += quit;
      criadas.push({ id: nova.id, competencia: comp, incrementados: inc, quitados: quit });
      comp = nextComp(comp);
    }
    appendAudit({
      categoria: "margem", acao: "simular_meses", userId: ator, userRole: "averbadora",
      detalhes: `Simulou avanco de ${body.meses} meses em ${pref.nome}: ${criadas.length} folhas criadas+consolidadas, ${totalIncrementados} parcelas incrementadas, ${totalQuitados} contratos quitados.`,
    });
    pushEvent("info", "admin.folhas.simular_meses", `${pref.nome}: +${body.meses} meses (${totalIncrementados} parcelas, ${totalQuitados} quitados)`);
    return c.json({ prefeitura: pref.nome, folhas: criadas, totalIncrementados, totalQuitados });
  })
